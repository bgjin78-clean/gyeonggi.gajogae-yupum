...
nearby = {
    "수원시":["용인시","화성시","오산시"], "성남시":["용인시","광주시","하남시"], "고양시":["파주시","김포시","양주시"],
    "용인시":["수원시","성남시","화성시"], "부천시":["광명시","시흥시","김포시"], "안산시":["시흥시","화성시","군포시"],
    "안양시":["군포시","의왕시","광명시"], "남양주시":["구리시","하남시","양평군"], "화성시":["수원시","오산시","평택시"],
    "평택시":["화성시","오산시","안성시"], "의정부시":["양주시","포천시","남양주시"], "시흥시":["안산시","광명시","부천시"],
    "파주시":["고양시","김포시","양주시"], "김포시":["고양시","파주시","부천시"], "광명시":["부천시","시흥시","안양시"],
    "광주시":["성남시","하남시","이천시"], "군포시":["안양시","의왕시","안산시"], "오산시":["수원시","화성시","평택시"],
    "이천시":["광주시","여주시","안성시"], "양주시":["의정부시","동두천시","포천시"], "구리시":["남양주시","하남시","의정부시"],
    "안성시":["평택시","이천시","용인시"], "포천시":["의정부시","양주시","동두천시"], "의왕시":["안양시","군포시","수원시"],
    "하남시":["남양주시","성남시","광주시"], "여주시":["이천시","양평군","광주시"], "동두천시":["양주시","포천시","연천군"],
    "과천시":["안양시","의왕시","성남시"], "가평군":["남양주시","양평군","포천시"], "양평군":["남양주시","여주시","가평군"],
    "연천군":["동두천시","포천시","양주시"]
}
slug_by_name = {name: slug for name, slug, *_ in cities}

photo_sets = [
    ([3,7,12,18],[4,9,15,22]),([5,11,18,24],[3,8,16,23]),([2,9,17,26],[5,10,18,24]),([4,13,21,29],[2,11,19,25]),
    ([6,14,22,30],[6,12,20,26]),([1,8,16,27],[7,13,21,25]),([10,15,23,28],[1,14,22,26]),([8,16,21,30],[5,12,18,24])
]
case_templates = [
    "{name} 현장에서는 먼저 가족분과 보관할 물품의 기준을 정한 뒤 서류와 사진, 귀중품을 확인했습니다. 이후 생활용품과 폐기물을 구분해 정리 범위를 나누고, 현장 동선에 맞춰 작업을 진행했습니다.",
    "{name} 유품정리 상담에서는 공간 규모와 폐기물 양을 먼저 확인했습니다. 엘리베이터 사용 가능 여부와 차량 진입 위치를 함께 확인해 작업 시간을 줄이고, 중요 물품은 별도로 모아 가족분께 안내했습니다.",
    "{name} 현장은 보관품과 폐기품이 섞여 있어 분류 과정이 중요했습니다. 통장, 도장, 사진, 계약서 등 확인이 필요한 물품을 먼저 구분하고 생활폐기물은 별도로 정리해 반출 계획을 세웠습니다.",
    "{name} 작업은 현장 확인 후 정리 순서를 정하는 방식으로 진행했습니다. 가구와 생활용품의 양이 많아 폐기물처리 상담을 함께 진행했고, 마무리 단계에서 공간 상태를 다시 확인했습니다."
]
faq_sets = [
...
def faq_html(name, start):
    selected = [faq_sets[(start+i) % len(faq_sets)] for i in range(4)]
    return "\n".join(f'        <details{" open" if i == 0 else ""}><summary>{q.format(name=name)}</summary><p>{a}</p></details>' for i, (q,a) in enumerate(selected))

def make_page(name, slug, feature, cost_note, idx):
    before_nums, process_nums = photo_sets[idx % len(photo_sets)]
    before = "\n".join(f'            <img src="images/before-{two(n)}.jpg" alt="{name} 유품정리 작업 전 사진 {i+1}">' for i,n in enumerate(before_nums))
    process = "\n".join(f'            <img src="images/process-{two(n)}.jpg" alt="{name} 유품정리 작업 중 사진 {i+1}">' for i,n in enumerate(process_nums))
    after = "\n".join(f'            <img src="images/after-{two(n)}.jpg" alt="{name} 유품정리 작업 후 사진 {i+1}">' for i,n in enumerate(before_nums))
    case_text = case_templates[idx % len(case_templates)].format(name=name)
...
<head>
  ...
  <meta property="og:title" content="{name} 유품정리 | 가족애유품정리"><meta property="og:description" content="{name} 유품정리, 고독사청소, 특수청소, 폐기물처리 상담. 가족의 마음으로 정리합니다."><meta property="og:type" content="website"><meta property="og:url" content="{url}"><meta property="og:image" content="{BASE_DOMAIN}/images/main-banner.png">
  <meta name="twitter:card" content="summary_large_image"><link rel="icon" href="/images/favicon.png"><link rel="stylesheet" href="style.css"><script src="https://cdn.jsdelivr.net/npm/@emailjs/browser@4/dist/email.min.js"></script>
  <script type="application/ld+json">{{"@context":"https://schema.org","@type":"LocalBusiness","name":"가족애유품정리 {name}","url":"{url}","telephone":"{PHONE}","image":"{BASE_DOMAIN}/images/main-banner.png","areaServed":"경기도 {name}","priceRange":"유품정리 25만원부터, 고독사청소 80만원부터, 폐기물처리 1톤 기준 25만원부터","description":"{name} 유품정리, 고독사청소, 특수청소, 폐기물처리 상담"}}</script>
</head>
<body>
<header class="topbar"><div class="container nav"><a class="logo" href="/">가족애<span>유품정리</span></a><nav class="navlinks"><a href="/#service">서비스</a><a href="/#price">비용안내</a><a href="/#process">진행과정</a><a href="/#areas">지역안내</a><a href="#consult">상담접수</a></nav><a class="call" href="tel:{PHONE}">{PHONE}</a></div></header>
<main>
...
<section id="service"><div class="container"><h2 class="sectionTitle">{name} 유품정리 서비스</h2><p class="sectionLead">현장 상황에 따라 유품정리, 고독사청소, 특수청소, 폐기물처리를 함께 안내합니다.</p><div class="grid4"><div class="card"><h3>유품정리</h3><p>남겨진 생활용품과 의류, 서류, 사진, 귀중품을 가족과 협의하며 분류합니다.</p></div><div class="card"><h3>고독사청소</h3><p>오염과 악취가 있는 공간은 일반 정리와 별도로 특수청소 필요 여부를 확인합니다.</p></div><div class="card"><h3>특수청소</h3><p>장기간 방치, 오염, 악취 등 일반 정리로 어려운 현장을 확인 후 안내합니다.</p></div><div class="card"><h3>폐기물처리</h3><p>가구, 가전, 생활폐기물, 잔짐 등 현장에서 발생한 폐기물 처리를 상담합니다.</p></div></div></div></section>
...
<section id="cost-factor"><div class="container"><h2 class="sectionTitle">{name} 현장에서 비용이 달라지는 이유</h2><p class="sectionLead">{cost_note}</p><div class="grid3"><div class="card"><h3>물품 양</h3><p>보관품과 폐기품의 양이 많을수록 작업 시간과 인력이 달라집니다.</p></div><div class="card"><h3>층수와 동선</h3><p>엘리베이터 여부, 주차 위치, 운반 동선에 따라 작업 난이도가 달라집니다.</p></div><div class="card"><h3>폐기물 양</h3><p>가구, 가전, 생활폐기물의 양과 종류에 따라 처리 비용이 달라질 수 있습니다.</p></div><div class="card"><h3>특수청소 여부</h3><p>오염, 악취, 장기간 방치가 있는 경우 추가 작업이 필요할 수 있습니다.</p></div><div class="card"><h3>작업 인원</h3><p>공간 규모와 희망 일정에 따라 투입 인원과 차량 수가 달라질 수 있습니다.</p></div><div class="card"><h3>일정 조율</h3><p>당일 상담이나 급한 일정은 현장 위치와 예약 상황에 따라 안내드립니다.</p></div></div></div></section>
<section id="process" class="areaBox"><div class="container"><h2 class="sectionTitle">{name} 유품정리 진행 과정</h2><p class="sectionLead">상담부터 마무리 확인까지 가족과 협의하며 단계별로 진행합니다.</p><div class="processGrid"><div class="card processCard"><div class="processHead"><span>1</span><h3>상담 접수</h3></div><p>주소, 공간 형태, 정리 범위, 희망 일정을 확인합니다.</p></div><div class="card processCard"><div class="processHead"><span>2</span><h3>현장 확인</h3></div><p>물품 양, 층수, 엘리베이터 여부를 확인합니다.</p></div><div class="card processCard"><div class="processHead"><span>3</span><h3>중요품 분류</h3></div><p>서류, 통장, 사진, 귀중품 등을 분류합니다.</p></div><div class="card processCard"><div class="processHead"><span>4</span><h3>유품정리</h3></div><p>보관품, 확인품, 폐기품을 구분합니다.</p></div><div class="card processCard"><div class="processHead"><span>5</span><h3>마무리 확인</h3></div><p>정리 후 공간 상태를 확인합니다.</p></div></div></div></section>
<section id="case" class="areaBox"><div class="container"><h2 class="sectionTitle">{name} 유품정리 상담 사례</h2><p class="sectionLead">{case_text}</p><div class="notice">현장마다 가족이 보관하고 싶은 물품, 폐기해야 할 물품, 확인이 필요한 서류가 다르기 때문에 상담 단계에서 기준을 먼저 정하는 것이 중요합니다.</div></div></section>
<section id="photos"><div class="container"><h2 class="sectionTitle">{name} 유품정리 작업 사진</h2><p class="sectionLead">작업 전·중·후 사진을 구분해 현장 정리 흐름을 확인할 수 있도록 구성했습니다.</p><div class="photoBlock"><h3>작업 전</h3><div class="gallery">
{before}
...
<section id="areas" class="areaBox"><div class="container"><h2 class="sectionTitle">{name} 인근 지역 상담 안내</h2><p class="sectionLead">{name} 외에도 경기 주요 지역 유품정리 상담을 함께 안내합니다.</p><div class="areaLinks">
{make_links(name)}
        </div></div></section>
...
def main():
    ...
    print("경기 31개 시군 SEO 강화 페이지 생성 완료")
